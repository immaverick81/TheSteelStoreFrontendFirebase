...
export class RegisterUserComponent implements OnInit {
  ...
  onClickRegisterUser() {
    this.userService.addUser(this.userform.controls['name'].value,
      this.userform.controls['password'].value,
      this.userform.controls['emailId'].value,
      this.userform.controls['birthDate'].value).subscribe(res => {
        if (res) {
          this.router.navigate(['/login']);
          this.toastService.addSingle('success', '', 'User registered successfully.');
        }
      });
  }
